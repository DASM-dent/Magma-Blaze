...
export function productAvailabilityWhatsappUrl(product: AvailabilityProduct) {
  const productUrl = getProductUrl(product.slug);
  const message = `Hola Magma Blaze, quiero verificar disponibilidad de "${product.name}".${productUrl ? ` Link: ${productUrl}` : ""}`;
...
export function cartAvailabilityWhatsappUrl(
  ...
  const total = `${symbol} ${subtotal.toLocaleString("es-DO")}`;
  const details = extraLines.filter(Boolean);
  const detailsBlock = details.length
    ? `\n\nDetalles de entrega:\n${details.map((line) => `- ${line}`).join("\n")}`
    : "";
  const message = `Hola Magma Blaze, quiero verificar disponibilidad de estos productos:\n${lines.join("\n")}\nSubtotal aproximado: ${total}${detailsBlock}`;
...
export function cartOrderWhatsappUrl(
  ...
  const sharedLine = sharedCartUrl
    ? `\n\nEnlace del carrito: ${sharedCartUrl}`
    : "";

  const message = `Hola Magma Blaze, quiero hacer este pedido:\n\n${lines.join("\n\n")}\n\nTotal aproximado: ${total}${sharedLine}${detailsBlock}`;
