...
export default function CuentaPage() {
  ...
  const provinceOptions = addressForm.country === 'RD' ? DOMINICAN_PROVINCES : US_STATES;
  const profileRegionOptions = profileForm.country === 'RD' ? DOMINICAN_PROVINCES : US_STATES;
  const cityOptions = addressForm.country === 'RD'
    ? municipalitiesFor(addressForm.province)
  ...
  const addAddress = useMutation({
    mutationFn: () => {
      if (!addressForm.province || !addressForm.city) throw new Error(addressForm.country === 'RD' ? 'Selecciona la provincia y el municipio.' : 'Selecciona el estado y escribe la ciudad.');
      if (addressForm.line1.trim().length < 5) throw new Error('Escribe una dirección más completa.');
      return api('/account/addresses', { method: 'POST', body: JSON.stringify(addressForm) });
    },
    onSuccess: () => {
    ...
    onError: (error: any) => setMsg(error.message || t('addresses.saveError')),
  });
      ...
      <div className="grid gap-6 lg:grid-cols-[250px_1fr]">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {section === 'addresses' && <div className="space-y-5"><h2 className="text-2xl font-semibold">{t('account.addresses')}</h2><div className="grid gap-4 md:grid-cols-2"><Field label={t('addresses.country')}><select className="input-dark" value={addressForm.country} onChange={e => setAddressForm({ ...addressForm, country: e.target.value, province: '', city: '', zip: e.target.value === 'US' ? '' : addressForm.zip })}><option value="RD">{t('common.countryRD')}</option><option value="US">{t('common.countryUS')}</option></select></Field><Field label={addressForm.country === 'RD' ? 'Provincia' : 'Estado'} hint={addressForm.country === 'RD' ? 'Selecciona una provincia de República Dominicana.' : 'Selecciona el estado de la dirección.'}><select className="input-dark" value={addressForm.province} onChange={e => setAddressForm({ ...addressForm, province: e.target.value, city: '' })}><option value="">Selecciona {addressForm.country === 'RD' ? 'una provincia' : 'un estado'}</option>{provinceOptions.map(p => <option key={p} value={p}>{p}</option>)}</select></Field><Field label={addressForm.country === 'RD' ? 'Municipio' : 'Ciudad'} hint={addressForm.country === 'RD' ? 'La lista corresponde a la provincia seleccionada.' : 'Escribe la ciudad de la dirección.'}>{addressForm.country === 'RD' ? <select className="input-dark" value={addressForm.city} disabled={!addressForm.province} onChange={e => setAddressForm({ ...addressForm, city: e.target.value })}><option value="">{addressForm.province ? 'Selecciona un municipio' : 'Primero selecciona una provincia'}</option>{cityOptions.map(c => <option key={c} value={c}>{c}</option>)}</select> : <input className="input-dark" value={addressForm.city} onChange={e => setAddressForm({ ...addressForm, city: e.target.value })} placeholder="Ej. Miami" />}</Field><Field label={t('addresses.zip')}><input className="input-dark" value={addressForm.zip} onChange={e => setAddressForm({ ...addressForm, zip: e.target.value })} /></Field><div className="md:col-span-2"><Field label={t('addresses.full')}><input className="input-dark" value={addressForm.line1} onChange={e => setAddressForm({ ...addressForm, line1: e.target.value })} placeholder={t('addresses.placeholder')} /></Field></div><label className="flex items-center gap-2 text-sm text-white/60"><input type="checkbox" checked={addressForm.isDefault} onChange={e => setAddressForm({ ...addressForm, isDefault: e.target.checked })} /> {t('addresses.default')}</label></div><button className="btn-ember" onClick={() => addAddress.mutate()}>{t('addresses.add')}</button><div className="grid gap-3">{addressQuery.data?.map((a: any) => <div key={a.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[.035] p-4"><div><b>{a.line1}</b><p className="text-sm text-white/45">{a.city} · {a.province} · {a.country} {a.zip ? `· ${a.zip}` : ''} {a.isDefault ? `· ${t('common.default')}` : ''}</p></div><button className="btn-ghost" onClick={() => deleteAddress.mutate(a.id)}>{t('common.delete')}</button></div>)}</div></div>}

            {section === 'locale' && <div className="space-y-5"><h2 className="text-2xl font-semibold">{t('account.locale')}</h2><div className="grid gap-4 md:grid-cols-2"><Field label={t('addresses.country')}><select className="input-dark" value={profileForm.country} onChange={e => setProfileForm({ ...profileForm, country: e.target.value, region: '', currency: e.target.value === 'US' ? 'USD' : 'DOP' })}><option value="RD">{t('common.countryRD')}</option><option value="US">{t('common.countryUS')}</option></select></Field><Field label={profileForm.country === 'RD' ? 'Provincia' : 'Estado'}><select className="input-dark" value={profileForm.region} onChange={e => setProfileForm({ ...profileForm, region: e.target.value })}><option value="">Selecciona {profileForm.country === 'RD' ? 'una provincia' : 'un estado'}</option>{profileRegionOptions.map(region => <option key={region} value={region}>{region}</option>)}</select></Field><Field label={t('locale.currency')}><select className="input-dark" value={profileForm.currency} onChange={e => setProfileForm({ ...profileForm, currency: e.target.value })}><option value="DOP">RD$ / DOP</option><option value="USD">US$ / USD</option></select></Field></div><p className="text-sm text-white/45">{t('locale.current')}: {country} / {currency}</p><button className="btn-ember" onClick={() => saveProfile.mutate({ country: profileForm.country, region: profileForm.region, language: 'es', currency: profileForm.currency })}>{t('common.save')}</button></div>}
